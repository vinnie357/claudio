defmodule TaskAppWeb.TaskLive.Index do
  ...
  def mount(_params, session, socket) do
    ...
    # Initialize theme from session or default to light mode
    ...
    # Push initial theme state to client
    socket = if connected?(socket) do
      push_event(socket, "update_theme", %{dark_mode: dark_mode})
    else
      socket
    end
    ...
    <div class="max-w-4xl mx-auto p-6">
      <header class="mb-8 flex items-center justify-between">
        ...
        <button
          ...
          class="p-3 rounded-lg bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors duration-200 shadow-sm"
        ...
        >
          ...
          <% else %>
            ...
            <svg class="w-5 h-5 text-gray-600 dark:text-gray-300" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
          ...
          <div class="flex gap-3">
            <div class="flex-1">
              <input
                ...
                class={"px-4 py-2 w-full border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent dark:bg-gray-700 dark:text-white transition-colors duration-200 #{if @error_message, do: "border-red-500 dark:border-red-400", else: "border-gray-300 dark:border-gray-600"}"}
                maxlength="100"
              />
              <%= if @error_message do %>
      ...
      <div class="grid md:grid-cols-3 gap-6 mb-6">
        <div class="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg text-center transition-colors duration-200">
          <div class="text-2xl font-bold text-blue-600 dark:text-blue-400"><%= @stats.total %></div>
          <div class="text-blue-800 dark:text-blue-300">Total Tasks</div>
        </div>
        <div class="bg-green-50 dark:bg-green-900/20 p-4 rounded-lg text-center transition-colors duration-200">
        ...
        <div class="bg-yellow-50 dark:bg-yellow-900/20 p-4 rounded-lg text-center transition-colors duration-200">
      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md transition-colors duration-200">
        ...
        <div class="p-6">
          ...
          <% else %>
            <div class="space-y-3">
              <%= for task <- @tasks do %>
                <div class={"flex items-center gap-3 p-3 rounded-lg transition-colors duration-200 #{if task.completed, do: "bg-green-50 dark:bg-green-900/20", else: "bg-gray-50 dark:bg-gray-700"}"}>
                  <button
                    phx-click="complete_task"
                    phx-value-id={task.id}
                    class={"w-5 h-5 border-2 rounded transition-colors duration-200 #{if task.completed, do: "bg-green-500 border-green-500", else: "border-gray-300 dark:border-gray-600 hover:border-green-400"}"}
                  >
                  ...
                  <div class="flex-1">
                    <span class={"transition-colors duration-200 #{if task.completed, do: "line-through text-gray-500 dark:text-gray-400", else: "text-gray-900 dark:text-white"}"}>
                      <%= task.title %>
                    </span>
                  </div>
